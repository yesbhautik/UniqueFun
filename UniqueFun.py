...
def do_zip_update():
    success = False
    if DEBUG_MODE:
        zip_url = "https://github.com/yesbhautik/UniqueFun/archive/dev.zip"
        dir_name = "BBomb-dev"
    else:
        zip_url = "https://github.com/yesbhautik/UniqueFun/archive/master.zip"
        dir_name = "BBomb-master"
    ...
    if success:
        mesgdcrt.SuccessMessage("BBomb was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update BBomb.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/yesbhautik/UniqueFun.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/yesbhautik/UniqueFun.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get(
        "https://raw.githubusercontent.com/yesbhautik/UniqueFun/master/.version"
    ).text.strip()
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("BBomb is up-to-date")
        mesgdcrt.GeneralMessage("Starting BBomb")
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("BBomb was created by @yesbhautik")
...
def selectnode(mode="sms"):
    ...
    try:
        ...
        max_limit = {"sms": 1000, "call": 150, "mail": 600}
        cc, target = "", ""
        if mode in ["sms", "call"]:
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("BBomb will work only in Python v3")
    sys.exit()
...
description = """BBomb - Your Friendly Spammer Application

BBomb can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

BBomb is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by @yesbhautik !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start BBomb with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start BBomb with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start BBomb with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update BBomb")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current BBomb contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current BBomb version")
